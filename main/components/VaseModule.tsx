...
import { restoreVase } from '../services/gemini';
import LoadingSpinner from './LoadingSpinner';
import BeforeAfterSlider from './BeforeAfterSlider';
import { translations, Language } from '../translations';

interface VaseModuleProps {
  lang: Language;
}

const VaseModule: React.FC<VaseModuleProps> = ({ lang }) => {
  ...
  const handleRestore = async () => {
    ...
    try {
      const result = await restoreVase(inputImage, context);
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-serif text-arch-stone mb-2">{t.vaseTitle}</h2>
        <p className="text-arch-clay italic">{t.vaseSubtitle}</p>
      </div>
      ...
      {inputImage && !loading && !resultImage && (
        <div className="max-w-lg mx-auto mb-8 animate-fade-in">
          ...
          <textarea
            ...
            placeholder={t.vaseContextPlaceholder}
          ...
          />
          <p className="text-xs text-arch-clay mt-1 text-right italic">{t.vaseContextHint}</p>
        </div>
      )}
      ...
      {inputImage && !loading && !resultImage && (
        <div className="flex justify-center mb-8">
          <button
            ...
            className="px-8 py-3 bg-amber-800 text-white font-serif text-lg rounded shadow-lg hover:bg-amber-900 transition-all border-2 border-arch-gold flex items-center gap-2"
          >
            <span>🏺</span> {t.vaseBtn}
          </button>
        </div>
      ...
      {resultImage && inputImage && (
        <div className="mt-8 animate-fade-in">
          <h3 className="text-xl font-serif text-arch-stone mb-4 text-center border-b border-arch-gold/30 pb-2">{t.vaseResultTitle}</h3>
          
          <div className="mx-auto max-w-3xl">
            <BeforeAfterSlider 
              beforeImage={inputImage}
              afterImage={resultImage}
              labelBefore={t.vaseBroken}
              labelAfter={t.vaseRestored}
            />
          </div>
          ...
          <div className="flex justify-center gap-4 mt-4">
             ...
             <a href={resultImage} download="vase-restored.png" className="text-arch-clay underline text-sm hover:text-arch-stone font-serif flex items-center">
               {t.download}
             </a>
          </div>
        </div>
