...
import { restoreAncientPlace } from '../services/gemini';
...
const RestorationModule: React.FC<RestorationModuleProps> = ({ lang }) => {
  ...
  const handleRestore = async () => {
    ...
    try {
      const result = await restoreAncientPlace(inputImage, context);
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-serif text-arch-stone mb-2">{t.restorationTitle}</h2>
        <p className="text-arch-clay italic">{t.restorationSubtitle}</p>
      </div>
      ...
      {inputImage && !loading && !resultImage && (
        <div className="max-w-lg mx-auto mb-8 animate-fade-in">
          <label className="block text-arch-stone font-serif font-bold mb-2 text-left">
            {t.restorationContextLabel}
          </label>
          <textarea
            value={context}
            onChange={(e) => setContext(e.target.value)}
            placeholder={t.restorationContextPlaceholder}
          ...
          />
          <p className="text-xs text-arch-clay mt-1 text-right italic">{t.restorationContextHint}</p>
        </div>
      )}
      ...
      {inputImage && !loading && !resultImage && (
        <div className="flex justify-center mb-8">
          <button
            ...
            className="px-8 py-3 bg-arch-clay text-white font-serif text-lg rounded shadow-lg hover:bg-stone-700 transition-all border-2 border-arch-gold flex items-center gap-2"
          >
            <span>✨</span> {t.restorationBtn}
          </button>
        </div>
      )}
      ...
      {resultImage && inputImage && (
        <div className="mt-8 animate-fade-in">
          <h3 className="text-xl font-serif text-arch-stone mb-4 text-center border-b border-arch-gold/30 pb-2">{t.restorationResultTitle}</h3>
          
          <div className="mx-auto max-w-3xl">
             <BeforeAfterSlider 
               beforeImage={inputImage}
               afterImage={resultImage}
               labelBefore={t.original}
               labelAfter={t.restored}
             />
          </div>
          ...
          <div className="flex justify-center gap-4 mt-2">
             <button 
               onClick={() => {
                 setResultImage(null);
                 // Keep input image and context
               }}
               className="text-arch-stone border border-arch-stone/30 px-4 py-1 rounded hover:bg-arch-sand/20 text-sm font-serif"
             >
               {t.tryAgain}
             </button>
             <a href={resultImage} download="restored-site.png" className="text-arch-clay underline text-sm hover:text-arch-stone font-serif flex items-center">
               {t.download}
             </a>
          </div>
        </div>
...
export default RestorationModule;